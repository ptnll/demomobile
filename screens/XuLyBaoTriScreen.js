...
export default function XuLyBaoTriScreen({ route, navigation }) {
  ...
  // Danh sách tùy chọn
  const techOptions = ['Nguyễn Văn A', 'Trần Văn B', 'Lê Thị C', 'Nguyễn Trần D', 'Phạm Thị E', 'Xem thêm...'];
  const statusOptions = ['Hoàn thành', 'Đã duyệt', 'Chờ duyệt', 'Từ chối'];
  ...
  const handleUpdate = () => {
    Alert.alert('Thành công', 'Đã cập nhật yêu cầu bảo trì!', [
      ...
      // Ở dự án thực tế, bạn sẽ truyền tham số updatedRequest về trang trước giống như trang Thiết Bị
    ]);
  };
      ...
      <View style={styles.contentWrapper}>
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <View style={[styles.inputGroup, { zIndex: 10 }]}>
            ...
            <TouchableOpacity 
              style={[styles.dropdownBox, { backgroundColor: currentStatusStyle.bg, borderColor: '#D69E2E', borderWidth: 1 }]} 
              onPress={() => setStatusOpen(!isStatusOpen)}
              activeOpacity={0.8}
            >
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <MaterialCommunityIcons name={currentStatusStyle.icon} size={18} color={currentStatusStyle.text} style={{ marginRight: 8 }} />
                <Text style={[styles.dropdownText, { color: currentStatusStyle.text, fontWeight: 'bold' }]}>
                  {request.status}
                </Text>
              </View>
            ...
            {isStatusOpen && (
              <View style={[styles.dropdownMenuList, { position: 'absolute', top: 75, left: 0, right: 0, zIndex: 99 }]}>
                {statusOptions.map((opt) => (
                  <TouchableOpacity 
                    key={opt} style={styles.dropdownMenuItem} 
                    onPress={() => { setRequest({...request, status: opt}); setStatusOpen(false); }}
                  >
                    <Text style={[styles.dropdownMenuText, request.status === opt && { fontWeight: 'bold', color: '#2B6CB0' }]}>{opt}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            )}
          ...
          {/* ================= GHI CHÚ / KẾT QUẢ ================= */}
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Ghi chú / Kết quả sửa chữa</Text>
            <View style={styles.textAreaContainer}>
              <TextInput
                style={styles.textArea}
                multiline
                numberOfLines={5}
                placeholder="Thêm ghi chú sửa chữa chi tiết cho Kỹ thuật viên..."
                placeholderTextColor="#A0AEC0"
                value={request.note}
                onChangeText={(t) => setRequest({...request, note: t})}
                textAlignVertical="top"
              />
              <MaterialCommunityIcons name="pencil" size={18} color="#A0AEC0" style={styles.pencilIcon} />
            </View>
          </View>
